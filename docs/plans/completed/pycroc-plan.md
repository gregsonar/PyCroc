...
- [x] перезапустить все тесты: `pytest -q` *(121 passed)*
- [x] запустить `ruff check .`
- [x] запустить `mypy src --strict`
- [x] вручную проверить (реальный `croc`, если установлен локально) happy-path отправки
      небольшого файла самому себе через `127.0.0.1` relay или два терминала на одной машине
      *(выполнено скриптованно с croc v10.2.7: найдены и исправлены 3 несовместимости —
      порядок глобальных флагов, piped stdin, формат прогресса v10; см. notes.md §5–7)*
- [x] убедиться, что приложение не падает при отсутствии установленного `croc` (показывает
      предупреждение в Settings) *(автотест test_missing_croc_warns_and_disables_transfer_tabs)*
...
- [x] описать в `README.md`: установку (`pip install pycroc`), требования (установленный
      бинарник `croc`), скриншот/описание вкладок, горячие клавиши
      *(установка описана из исходников — публикации на PyPI ещё не было, см. Post-Completion;
      вместо скриншота — описание вкладок; добавлено предупреждение о хранении кодов
      открытым текстом — решение по пункту 3 ручного тестирования)*
- [x] описать формат профилей (`~/.config/pycroc/config.toml`) для ручного редактирования
- [x] добавить `CHANGELOG.md` с записью `0.1.0 — initial release`
- [x] тестовых изменений не требуется

### Task 16: Финальный wrap-up

- [x] зафиксировать в `docs/notes.md` неочевидный готча: буферизация прогресс-строк croc по
      `\r` (см. Task 3) — единственный найденный при планировании неочевидный момент
      *(итог: в notes.md 17 готчей — croc v10 добавил ещё немало, см. §5–7)*
- [x] переместить этот план в `docs/plans/completed/pycroc-plan.md`
- [x] тестовых изменений не требуется
