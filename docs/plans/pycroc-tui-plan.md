...
- [ ] реализовать список профилей с созданием/редактированием/удалением
- [ ] реализовать выбор активного профиля (сохраняется через `ConfigStore`)
- [ ] реализовать поле пути к бинарнику croc с кнопкой "проверить" (`check_binary()`)
- [ ] добавить Pilot-тесты: создание профиля появляется в списке, попытка удалить `default`
      показывает ошибку, а не падает
- [ ] запустить `pytest tests/ui/widgets/test_settings_panel.py -q`
